...
module.exports = async (req, res, next) => {
  ...
  if (!authorization) {
    await LogsCreator(null, null, 'Doctor Middlerware', 'error', 'Authorization not found!');
  ...
  if (!administrationAccount) {
    await LogsCreator(User, null, 'Doctor Middlerware', 'error', 'This administration account not found!');
  ...
  if (administrationAccount.role !== 'doctor' && administrationAccount.role !== 'super-admin') {
    await LogsCreator(User, null, 'Doctor Middlerware', 'error', 'This administration account doesn\'t have authorization for this endpoint!');
  ...
  if (!loginStatus) {
    await LogsCreator(User, null, 'Doctor Middlerware', 'error', 'This administration doesn\'t have login status!');
  ...
  if (administrationAccount.status === 'inactive') {
    await LogsCreator(User, null, 'Doctor Middlerware', 'error', 'This administration account status is inactive!');
  ...
  if (!loginStatus.loggedIn) {
    await LogsCreator(User, null, 'Doctor Middlerware', 'error', 'This administration account isn\'t currently logged in on any device!');
  ...
  if (loginStatus.lastUpdate.toString() !== administrationAccount.updatedAt.toString()) {
    await LogsCreator(User, null, 'Doctor Middlerware', 'error', 'This administration account recently updated, please re-login!');
